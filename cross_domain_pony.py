...
parser.add_argument("factor_pony", type=str)
...
eigvec2 = torch.load(args.factor_pony)["eigvec"].to(args.device)
...
V_t += dist
#vec_cross = torch.mm(vec1, eigvec1)
#vec_cross = torch.mm(vec_cross, V_t)
#vec_cross = torch.mm(vec_cross, eigvec2)
vec_cross = torch.mm(vec1, eigvec2)
...
img, _ = g_ema([vec_cross], truncation=1, truncation_latent=None) 
