...
parser.add_argument("factor_anime", type=str)
...
g_ema.load_state_dict(torch.load(args.model, map_location='cuda:0')["g_ema"], strict=False)
g_ema.eval()
...
eigvec2 = torch.load(args.factor_anime)["eigvec"].to(args.device)
...
#dist = eigvec2 - eigvec1 
#V_t = torch.mm(V_t, dist)
#vec_cross = torch.mm(vec1, V_t)
...
img, _ = g_ema([vec_cross], truncation=1, truncation_latent=mean_latent) 
